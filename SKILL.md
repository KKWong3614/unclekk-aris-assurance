...
配套脚本 `scripts/aris_audit.py`（单文件、零依赖、四子命令）把阶段 2/3 里**最机械、最该先做的部分**自动化，
让你在派 Hermes + SenseNova 异族审之前，先自己降一轮缺口。

The companion script `scripts/aris_audit.py` (single-file, zero-dependency, four subcommands) automates the **most mechanical, should-do-first parts** of stages 2/3, letting you reduce a round of gaps yourself before dispatching the Hermes + SenseNova out-group review.
...
- **单文件四子命令**：`extract`（阶段 2）+ `drift`（阶段 3）+ `gate`（收口硬闸门）+ `reconcile`（跨文档证据核验）。`gate` 在收口前扫描台账，只要还有未验证声明（证据列仍 `⚠️` 且未显式降级/删除）就返回**非零退出码**，从机制上阻止"带缺口发稿"。`reconcile` 用来源文档兜底核验"无来源支撑的断言"。
- **复杂格式识别**：extract/drift 已支持 Markdown 表格单元格、加粗（`**x**`）、项目符号、标题后声明的抽取，避免"复杂格式识别不全"导致的漏检。
- **脚本覆盖范围**：脚本仅自动化阶段 2（草稿→台账骨架）和阶段 3（漂移检测）。**阶段 1（完整性验证：文件是否齐全、口径是否统一、关键证据是否到位）是人工步骤**，在 Runbook ③ 中由人工或异族审完成，脚本不替代。（v2.0.0 起，`reconcile` 子命令提供**跨文档证据核验**，作为阶段 2/3 之外的「异族证据兜底」——专门抓"无来源支撑"的断言，仍不替代阶段 1 人工完整性验证。）
...
- **Single-file four subcommands**: `extract` (stage 2) + `drift` (stage 3) + `gate` (stage-4 closure hard-gate) + `reconcile` (cross-document evidence check).
...
配套回归测试 `scripts/test_aris_audit.py`（23 项，用固定 fixture 跑 extract/drift/gate/reconcile，断言台账含 ⚠️、父目录自动创建、非 UTF-8 干净报错、未闭环台账 gate 非零退出、漂移类型 B 弱声明被强化、标题后声明检测、**表格单元格/加粗声明识别**、`--json` 输出、目录/二进制/空/超大文件干净报错（退出码 2/3/4/7）、gate 退出码 10、**reconcile 跨文档证据核验（有/无来源支撑分检 + 严重度 P0/P1/P2 分级）**、`extract --json` 带 severity 分级等）。运行：

The companion regression test `scripts/test_aris_audit.py` (23 items, using fixed fixtures to run extract/drift/gate/reconcile, asserting the ledger contains ⚠️, parent directory auto-created, clean error on non-UTF-8, gate non-zero exit on unclosed ledger, drift type B weak claim strengthened, post-heading claim detection, **table-cell/bold claim recognition**, `--json` output, clean errors on directory/binary/empty/oversized files (exit codes 2/3/4/7), gate exit code 10, **reconcile cross-document check (supported/unsupported split + severity P0/P1/P2 tiers)**, `extract --json` severity tier, etc.). Run:
...
### reconcile（v2.0.0 跨文档证据核验）：草稿 + 来源 → 不支持声明清单 (reconcile — v2.0.0 cross-document evidence check)

```bash
python scripts/aris_audit.py reconcile --draft 成稿.md --source 论文.md 数据.csv.md --output reconcile.md
# 可选项：--json（输出 JSON 对象）   --threshold 0.3（声明与来源句子的 Jaccard 相似度阈值，默认 0.3）
```

输出跨文档证据核验报告：
Outputs a cross-document evidence report:

- **来源覆盖率**：草稿声明中能在来源文档里找到支撑（token Jaccard ≥ 阈值）的比例。
- **不支持声明清单**：覆盖率低于阈值的声明，按严重度优先排列：
  - `P0`：含数字/百分比/倍数的**量化声明**无来源 → 最高风险（最易被"看似成立"）。
  - `P1`：含"证明/必然/所有/一定/完全"等**强绝对词**的声明无来源 → 高风险。
  - `P2`：其他未验证声明 → 中低风险。

> 这是 extract/drift/gate 之外的**「异族证据兜底」**：即便台账自审通过，仍可能漏掉"无来源支撑"的断言，reconcile 专门抓这一类。覆盖度为轻量启发式（声明与来源句子的 token Jaccard），相似度低仅提示"来源中可能无直接支撑"，**不证明为假**；最终以人工/异族审为准。本工具不替代 Hermes+SenseNova 异族审。

> This is the **"out-group evidence backstop"** beyond extract/drift/gate: even if the ledger self-audit passes, claims with no source support can slip through; `reconcile` specifically catches these. Coverage is a lightweight heuristic (token Jaccard between claim and source sentences); low similarity only hints "source may lack direct support," **not proof of falsehood**; final call rests with human/out-group review.

### 可选项与错误处理（新手友好） (Options & Error Handling — Newbie-Friendly)

四个子命令共享 `--max-bytes`（文件大小上限，默认 5MB，超限报错 `ERR-TOO-LARGE`）。其余可选 flag：

| 子命令 | 可选 flag | 作用 |
|---|---|---|
| `extract` | `--json` | 输出 JSON 数组（每条声明含 id/claim/evidence/strength/gap/**severity**），便于 CI/程序消费 |
...
| `reconcile` | `--source <文件…>` | 来源文档（可多个，空格分隔），与草稿做跨文档证据核验 |
| `reconcile` | `--json` | 输出 JSON 对象（含 coverage_pct / supported / unsupported / severity_of_unsupported / items） |
| `reconcile` | `--threshold <浮点>` | 声明与来源句子的 Jaccard 相似度阈值，默认 0.3 |
...
All four subcommands share `--max-bytes` (file size cap, default 5MB; exceeds → `ERR-TOO-LARGE`). Other optional flags:

| subcommand | optional flag | effect |
|---|---|---|
| `extract` | `--json` | emit a JSON array (each claim has id/claim/evidence/strength/gap/**severity**) for CI/programmatic use |
...
| `reconcile` | `--source <files…>` | source documents (multiple, space-separated) to cross-check the draft against |
| `reconcile` | `--json` | emit a JSON object (coverage_pct / supported / unsupported / severity_of_unsupported / items) |
| `reconcile` | `--threshold <float>` | Jaccard similarity threshold between claim and source sentences, default 0.3 |
...
> 脚本级回归测试见 `scripts/test_aris_audit.py`（23 项，覆盖 extract/drift/gate/reconcile/父目录创建/编码错误/未闭环 gate 非零退出(10)/漂移类型B/标题后声明检测/表格单元格声明/加粗声明/`--json` 输出/目录·二进制·空·超大文件干净报错(2/3/4/7)/`--mark` 自定义标记/`extract --json` 严重度分级/reconcile 跨文档证据核验与 P0/P1/P2 分级等）。
...
> For script-level regression tests, see `scripts/test_aris_audit.py` (23 items, covering extract/drift/gate/reconcile/parent-dir creation/encoding errors/gate non-zero exit on unclosed ledger (10)/drift type B/post-heading claim detection/table-cell claim/bold claim/`--json` output/clean errors on directory·binary·empty·oversized files (2/3/4/7)/`--mark` custom marker/`extract --json` severity tier/reconcile cross-document check with P0/P1/P2 tiering, etc.).
...
覆盖范围：extract 台账生成、证据列 ⚠️ 标记、父目录自动创建、stdout 输出、drift 新增声明检测（类型A）、drift 弱声明被强化（类型B）、标题后声明检测（P1-1 回归）、文件不存在报错、**表格单元格声明识别、加粗声明识别、`--json` 输出、目录/二进制/空/超大文件干净报错（退出码 2/3/4/7）、`--mark` 自定义标记、gate 未闭环退出码 10、`extract --json` 严重度分级（P0/P1/P2）、reconcile 跨文档证据核验（有/无来源支撑分检 + 严重度分级）**。
